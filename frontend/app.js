...
async function handleLogin(event) {
  ...
  try {
    // Envia também `nomeUsuario` com o mesmo valor de `username` para compatibilidade
    ...
    // Sempre leia o texto da resposta para debug/feedback
    const respText = await response.text();

    if (response.ok) {
      showMsg(
        "loginSuccess",
        respText || "Login realizado com sucesso",
        "success"
      );
      ...
      // mostra status + mensagem do servidor para facilitar debug
      showMsg(
        "loginError",
        `Erro ao fazer login (${response.status}): ${
          respText || "Sem detalhes"
        }`
      );
    }
  } catch (error) {
...
async function handleRegister(event) {
  ...
  try {
    ...
    if (response.ok || response.status === 201) {
      ...
      showMsg(
        "registerError",
        error || "Username já existe ou dados inválidos"
      );
    } else {
...
async function loadProfile() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // 1. Nome Principal (em cima)
      document.getElementById("profileName").textContent = data.nomeUsuario;
      
      // 2. Dados da Lista (os que acabamos de criar no HTML)
      document.getElementById("displayNome").textContent = data.nomeUsuario;
      document.getElementById("displaySessao").textContent = data.codigoSessao;
      document.getElementById("displayHostname").textContent = data.hostnameServidor; // O mais importante!

      // 3. Formatação da Data
      if (data.dataLogin) {
        const dataObj = new Date(data.dataLogin);
        document.getElementById("displayData").textContent = dataObj.toLocaleString("pt-BR");
      }
      // ------------------------
